...
export default function SsgcsTab() {
  ...
  useEffect(() => {
    fetch("https://api.github.com/repos/SUAS-STEM/.gcs/releases/latest")
      ...
      .catch(() => {
        setRelease({ tag: "dev-v0.001", date: "", url: "https://github.com/SUAS-STEM/.gcs/releases" });
      });
  }, []);
  ...
  return (
    <div className="max-w-2xl">
      ...
      <p>
        The SUAS@STEM Ground Control Software (SSGCS) is used to control and monitor the Event
        Horizon aircraft during flight operations.{" "}
      ...
      <div className="mt-8">
        ...
        <div className="space-y-3 mt-4">
          {(["windows-x86", "macos-arm64"] as const).map((p) => (
            <label
              key={p}
              className="flex items-center p-4 border border-white/30 rounded-lg cursor-pointer hover:bg-white/5 transition"
              style={{ backgroundColor: selectedPlatform === p ? "rgba(255,255,255,0.1)" : "transparent" }}
            >
              <input
                type="radio"
                name="platform"
              ...
              <span className="ml-3">
                {p === "windows-x86" ? (
                  <>
                    ...
                    <span className="text-sm text-white/60">For 64-bit Intel and AMD processors</span>
                  </>
                ) : (
                  <>
                    <span className="block font-semibold">macOS (Apple Silicon)</span>
                    <span className="text-sm text-white/60">For Apple M-series processors</span>
                  </>
                )}
              </span>
            </label>
        ...
        <div className="mt-8 flex items-center gap-4 flex-wrap">
          ...
          {release && (
            <div className="text-sm text-white/50 font-mono">
              <a href={release.url} target="_blank" rel="noopener noreferrer" className="text-white/70 hover:text-white transition">
                {release.tag}
              </a>
              {release.date && <span className="ml-2 text-white/40">{release.date}</span>}
            </div>
          )}
        </div>
      </div>
        ...
        <table>
          ...
          <tbody>
            ...
            <tr>
              <td>CPU</td>
              <td>Intel or AMD</td>
              <td>Intel or AMD<br />11th generation+<br />8+ logical cores<br />2.00 GHz+</td>
            </tr>
            <tr><td>RAM</td><td>4 GB</td><td>16 GB</td></tr>
            <tr><td>Storage</td><td>500 MB free</td><td>16 GB free</td></tr>
          </tbody>
        </table>
      </div>
      ...
      <div className="mt-8">
        ...
        <p className="text-white/80">
          ...
          <a href="mailto:ethan@ethanchan.studio" className="text-blue-400 hover:underline">
            Ethan Chan
          </a>
          .
        </p>
