...
import { jobDir, MAX_FILES_PER_JOB, readJob, writeJob } from "@/lib/stitchJobs";
...
export async function POST(_req: Request, ctx: { params: Promise<{ id: string }> }) {
  ...
  await writeJob(meta);
  return NextResponse.json({ ok: true, id, files: meta.files.length, totalBytes: meta.totalBytes });
}
