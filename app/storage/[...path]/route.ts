...
function responseHeaders(response: Response): Headers {
  const headers = new Headers();
  for (const name of ["content-type", "content-length", "content-disposition", "etag", "cache-control", "accept-ranges"]) {
...
async function forward(request: NextRequest, context: RouteContext) {
  ...
  const upstream = await fetch(`${STORAGE_ORIGIN}/v1/files/${upstreamPath}${request.nextUrl.search}`, {
